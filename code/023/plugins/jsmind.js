...
; (function ($w) {
    ...
    jm.view_provider.prototype = {
        ...
        create_node_element: function (node, parent_node) {
            ...
            if (!!node.topic) {
                var htmlBefore = node.data.htmlBefore ? node.data.htmlBefore : '';
                var htmlBehind = node.data.htmlBehind ? node.data.htmlBehind : '';
                if (this.opts.support_html) {
                    $h(d, htmlBefore + node.topic + htmlBehind);
                } else {
                    $t(d, htmlBefore + node.topic + htmlBehind);
                }
            }

            d.setAttribute('nodeid', node.id);
            if (node.parent) {
                d.setAttribute('pnodeid', node.parent.id);
            }
            if (node.data.className) {
                d.className = node.data.className;
            }
            d.setAttribute('index', node.index);
            ...
            if (!node.isroot) {
                ...
                if (node.direction == -1) {
                    d_e.style.marginRight = '6px';
                    // d.prepend(d_e);
                    if (d.firstChild) {
                        d.insertBefore(d_e, d.firstChild);
                    } else {
                        d.appendChild(d_e);
                    }
                } else {
                    d_e.style.marginLeft = '6px';
                    d.appendChild(d_e);
                }
        ...
        show_nodes: function () {
            ...
            for (var nodeid in nodes) {
                ...
                if (!node.isroot && node.children.length == 0) {
                    if (node.direction == -1) {
                        expander.parentNode.style.left = parseFloat(expander.parentNode.style.left.split('px')[0]) + 19 + 'px'
                    }
                    expander.style.display = 'none';
